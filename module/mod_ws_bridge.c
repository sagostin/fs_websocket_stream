 ...
 * extended with full-duplex playback via SMBF_WRITE_REPLACE.
 */
#include "mod_ws_bridge.h"
#include "ws_bridge_glue.h"

SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_ws_bridge_shutdown);
SWITCH_MODULE_LOAD_FUNCTION(mod_ws_bridge_load);

SWITCH_MODULE_DEFINITION(mod_ws_bridge, mod_ws_bridge_load, mod_ws_bridge_shutdown, NULL);
                                     ...
                                     char* metadata)
{
    ...
    if (switch_channel_get_private(channel, MY_BUG_NAME)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_ws_bridge: bug already attached!\n");
        return SWITCH_STATUS_FALSE;
    }

    if (switch_channel_pre_answer(channel) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_ws_bridge: channel must have reached pre-answer status before calling start!\n");
        return SWITCH_STATUS_FALSE;
    }

    read_codec = switch_core_session_get_read_codec(session);

    /* WRITE_REPLACE is always attached so the bridge can inject downlink
     * audio for true full-duplex streaming. */
    ...
    if (SWITCH_STATUS_FALSE == stream_session_init(session, responseHandler, read_codec->implementation->actual_samples_per_second,
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Error initializing mod_ws_bridge session.\n");
        return SWITCH_STATUS_FALSE;
    }
...
static switch_status_t do_stop(switch_core_session_t *session, char* text)
{
    ...
    if (text) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_ws_bridge: stop w/ final text %s\n", text);
    }
    else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_ws_bridge: stop\n");
    }
...
static switch_status_t do_pauseresume(switch_core_session_t *session, int pause)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_ws_bridge: %s\n", pause ? "pause" : "resume");
...
static switch_status_t send_text(switch_core_session_t *session, char* text) {
    ...
    if (bug) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_ws_bridge: sending text: %s.\n", text);
        status = stream_session_send_text(session, text);
    }
    else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_ws_bridge: no bug, failed sending text: %s.\n", text);
    }
    return status;
}

#define STREAM_API_SYNTAX "<uuid> [start | stop | send_text | pause | resume] [ws-url] [mono | mixed | stereo] [8000 | 16000] [metadata]"
SWITCH_STANDARD_API(stream_function)
{
    ...
    assert(cmd);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "mod_ws_bridge cmd: %s\n", cmd ? cmd : "");
    ...
    if (zstr(cmd) || argc < 2 || (0 == strcmp(argv[1], "start") && argc < 4)) {
        ...
        if ((lsession = switch_core_session_locate(argv[0]))) {
            if (!strcasecmp(argv[1], "stop")) {
                ...
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                                  "unsupported mod_ws_bridge cmd: %s\n", argv[1]);
            }
            switch_core_session_rwunlock(lsession);
        } else {
...
SWITCH_MODULE_LOAD_FUNCTION(mod_ws_bridge_load)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_ws_bridge API loading..\n");
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    if (switch_event_reserve_subclass(EVENT_JSON) != SWITCH_STATUS_SUCCESS ||
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't register an event subclass for mod_ws_bridge API.\n");
        return SWITCH_STATUS_TERM;
    }
    SWITCH_ADD_API(api_interface, "uuid_ws_bridge", "ws_bridge API", stream_function, STREAM_API_SYNTAX);
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid start ws-url metadata");
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid start ws-url");
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid stop");
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid pause");
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid resume");
    switch_console_set_complete("add uuid_ws_bridge ::console::list_uuid send_text");

    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_ws_bridge API successfully loaded\n");
